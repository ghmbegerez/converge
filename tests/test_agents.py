...
def _setup_intent_with_risk(risk_score=30.0, damage_score=20.0):
...
class TestAgentAuthorization:
    def test_default_policy_blocks_merge(self, db_path):
        _setup_intent_with_risk()
        result = agents.authorize(
        agent_id="bot-1", action="merge",
    ...
    def test_configured_policy_allows(self, db_path):
        _setup_intent_with_risk(risk_score=20.0)
        pol = AgentPolicy(
            agent_id="bot-2",
            tenant_id="team-a",
            atl=2,
        ...
        agents.set_policy(pol)

        result = agents.authorize(
        agent_id="bot-2", action="merge",
            intent_id="agt-001", tenant_id="team-a",
        )
        assert result["allowed"]

    def test_risk_exceeds_agent_limit(self, db_path):
        _setup_intent_with_risk(risk_score=80.0)
        pol = AgentPolicy(
            agent_id="bot-3",
            tenant_id="team-a",
            max_risk_score=50.0,
    ...
    def test_human_approval_required(self, db_path):
        _setup_intent_with_risk(risk_score=10.0)
        pol = AgentPolicy(
            agent_id="bot-4",
            tenant_id="team-a",
            allow_actions=["merge"],
    ...
    def test_action_overrides(self, db_path):
        _setup_intent_with_risk(risk_score=45.0)
        pol = AgentPolicy(
            agent_id="bot-5",
            tenant_id="team-a",
            max_risk_score=60.0,
        ...
        result = agents.authorize(
        agent_id="bot-5", action="automerge",
            intent_id="agt-001", tenant_id="team-a",
        )
        assert not result["allowed"]
    ...
    def test_authorization_event_recorded(self, db_path):
        _setup_intent_with_risk()
        agents.authorize(
        agent_id="bot-6", action="analyze",
            intent_id="agt-001", tenant_id="team-a",
        )
        events = event_log.query(event_type="agent.authorized")
        assert len(events) >= 1
...
class TestPolicyCRUD:
    def test_set_and_get(self, db_path):
        ...
        agents.set_policy(pol)
        loaded = agents.get_policy("crud-bot", "team-a")
        assert loaded.atl == 3
        assert "merge" in loaded.allow_actions

    def test_list_policies(self, db_path):
        agents.set_policy(AgentPolicy(agent_id="b1", allow_actions=["a"]))
        agents.set_policy(AgentPolicy(agent_id="b2", allow_actions=["b"]))
        policies = agents.list_policies()
