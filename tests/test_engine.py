...
class TestValidateIntent:
    def test_validate_mergeable(self, db_path):
        ...
        events = event_log.query(event_type="intent.validated")
        assert len(events) == 1
    ...
    def test_trace_id_propagated_to_events(self, db_path):
        ...
        validated_events = event_log.query(event_type="intent.validated")
...
class TestProcessQueue:
    ...
    def test_invariant3_max_retries_rejects(self, db_path):
        ...
        max_retries=3, use_last_simulation=True, skip_checks=True
        )
        assert len(results) == 1
        assert results[0]["decision"] == "rejected"

        # Verify status changed
        loaded = event_log.get_intent("q-001")
    ...
    def test_queue_lock_prevents_concurrent(self, db_path):
        ...
        assert event_log.acquire_queue_lock(holder_pid=99999)
...
class TestConfirmMerge:
    def test_confirm_queued_intent(self, db_path):
        make_intent(id="m-001", status=Status.QUEUED)
        event_log.update_intent_status("m-001", Status.QUEUED)

        result = engine.confirm_merge("m-001", merged_commit="abc123")
        assert result["status"] == "MERGED"

        loaded = event_log.get_intent("m-001")
        assert loaded.status == Status.MERGED

    def test_confirm_nonexistent(self, db_path):
        result = engine.confirm_merge("nonexistent")
        assert "error" in result

    def test_confirm_wrong_status(self, db_path):
        make_intent(id="m-002", status=Status.MERGED)
        event_log.update_intent_status("m-002", Status.MERGED)

        result = engine.confirm_merge("m-002")
        assert "error" in result


class TestResetAndInspect:
    def test_reset_retries(self, db_path):
        make_intent(id="r-001", status=Status.READY)
        event_log.update_intent_status("r-001", Status.READY, retries=5)

        result = engine.reset_queue("r-001", set_status="VALIDATED")
        assert result["retries"] == 0
        assert result["status"] == "VALIDATED"

    def test_inspect_actionable(self, db_path):
        make_intent(id="i-001", status=Status.READY)
        make_intent(id="i-002", status=Status.VALIDATED)
        make_intent(id="i-003", status=Status.MERGED)

        result = engine.inspect_queue(only_actionable=True)
...
class TestRunChecks:
    ...
    def test_passing_check_records_event(self, db_path):
        ...
        with patch("converge.engine.subprocess.run") as mock_run:
            ...
            results = engine.run_checks(["lint"], intent_id="chk-001")
        ...
        events = event_log.query(event_type=EventType.CHECK_COMPLETED)
        assert len(events) == 1
    ...
    def test_failing_check_records_failure(self, db_path):
        ...
        with patch("converge.engine.subprocess.run") as mock_run:
            ...
            results = engine.run_checks(["lint"], intent_id="chk-002")
    ...
    def test_timeout_records_failure(self, db_path):
        ...
        with patch("converge.engine.subprocess.run") as mock_run:
            ...
            results = engine.run_checks(["lint"], intent_id="chk-003")
    ...
    def test_unsupported_check_skipped(self, db_path):
        ...
        results = engine.run_checks(["unknown_check_type"])
    ...
    def test_multiple_checks_all_recorded(self, db_path):
        ...
        with patch("converge.engine.subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(returncode=0, stdout="OK", stderr="")
            results = engine.run_checks(["lint", "unit_tests"], intent_id="chk-004")
    ...
    def test_file_not_found_records_failure(self, db_path):
        ...
        with patch("converge.engine.subprocess.run") as mock_run:
            ...
            results = engine.run_checks(["lint"], intent_id="chk-005")
...
class TestValidateWithChecks:
    ...
    def test_validate_passes_when_checks_pass(self, db_path):
        ...
        intent = make_intent(id="vc-001")
        sim = Simulation(mergeable=True, files_changed=["a.py"], source="feature/x", target="main")

        with patch("converge.engine.subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(returncode=0, stdout="OK", stderr="")
            result = engine.validate_intent(intent, sim=sim, skip_checks=False)

        assert result["decision"] == "validated"

        # Check events were recorded
        check_events = event_log.query(event_type=EventType.CHECK_COMPLETED)
        assert len(check_events) >= 1

    def test_validate_blocks_when_check_fails(self, db_path):
        """Invariant 1: mergeable ∧ ¬checks_pass → blocked."""
        intent = make_intent(id="vc-002")
        sim = Simulation(mergeable=True, files_changed=["a.py"], source="feature/x", target="main")
    ...
    def test_trace_id_propagated_to_simulation_and_checks(self, db_path):
        ...
        intent = make_intent(id="vc-trace")
        sim = Simulation(mergeable=True, files_changed=["a.py"], source="feature/x", target="main")
        ...
        check_events = event_log.query(event_type=EventType.CHECK_COMPLETED, intent_id="vc-trace")
        assert len(check_events) >= 1
        assert check_events[0]["trace_id"] == trace_id

        # Risk and policy events too
        risk_events = event_log.query(event_type=EventType.RISK_EVALUATED, intent_id="vc-trace")
        assert risk_events[0]["trace_id"] == trace_id

        policy_events = event_log.query(event_type=EventType.POLICY_EVALUATED, intent_id="vc-trace")
    ...
    def test_validate_high_risk_blocks_without_all_checks(self, db_path):
        ...
        intent = make_intent(id="vc-003", risk_level=RiskLevel.HIGH)
        sim = Simulation(mergeable=True, files_changed=["a.py"], source="feature/x", target="main")
