...
def _seed_events(db_path, n_sims=10, n_merged=5, n_rejected=2):
...
class TestRepoHealth:
    def test_basic_health(self, db_path):
        ...
        health = projections.repo_health(db_path, tenant_id="team-a")
    ...
    def test_health_event_recorded(self, db_path):
        _seed_events(db_path)
        projections.repo_health(db_path)
        events = event_log.query(db_path, event_type="health.snapshot")
        assert len(events) >= 1


class TestChangeHealth:
    def test_change_health(self, db_path):
        _seed_events(db_path, n_sims=3)
        result = projections.change_health(db_path, "int-000")
...
class TestCompliance:
    def test_compliance_passing(self, db_path):
        _seed_events(db_path, n_sims=10, n_rejected=0)
        report = projections.compliance_report(db_path)
        assert report.mergeable_rate >= 0.8
        assert report.passed is True

    def test_compliance_with_tenant_thresholds(self, db_path):
        _seed_events(db_path)
        event_log.upsert_compliance_thresholds(db_path, "team-a",
                                                {"min_mergeable_rate": 0.95})
        report = projections.compliance_report(db_path, tenant_id="team-a")
...
class TestTrends:
    def test_risk_trend(self, db_path):
        _seed_events(db_path)
        trend = projections.risk_trend(db_path, tenant_id="team-a")
        assert len(trend) > 0
        assert "risk_score" in trend[0]

    def test_entropy_trend(self, db_path):
        _seed_events(db_path)
        trend = projections.entropy_trend(db_path, tenant_id="team-a")
        assert len(trend) > 0
...
class TestQueueState:
    def test_queue_state(self, db_path):
        ...
        state = projections.queue_state(db_path)
...
class TestPredictions:
    def test_no_signals_when_healthy(self, db_path):
        ...
        _seed_events(db_path, n_sims=2, n_merged=2, n_rejected=0)
        signals = projections.predict_issues(db_path)
...
class TestIntegrationMetrics:
    def test_integration_metrics(self, db_path):
        _seed_events(db_path)
        metrics = projections.integration_metrics(db_path, tenant_id="team-a")
...
class TestPredictHealth:
    def _seed_health_snapshots(self, db_path, n=6, declining=False):
    ...
    def test_predict_insufficient_data(self, db_path):
        ...
        result = projections.predict_health(db_path, min_snapshots=3)
    ...
    def test_predict_stable_health(self, db_path):
        ...
        self._seed_health_snapshots(db_path, n=6, declining=False)
        result = projections.predict_health(db_path, tenant_id="team-a")
    ...
    def test_predict_declining_health(self, db_path):
        ...
        self._seed_health_snapshots(db_path, n=8, declining=True)
        result = projections.predict_health(db_path, tenant_id="team-a")
    ...
    def test_predict_event_recorded(self, db_path):
        ...
        self._seed_health_snapshots(db_path, n=4)
        projections.predict_health(db_path)
        events = event_log.query(db_path, event_type="health.prediction")
        assert len(events) >= 1
...
class TestStructuredLearning:
    def test_health_learning_structure(self, db_path):
        ...
        _seed_events(db_path, n_sims=10, n_rejected=5)
        health = projections.repo_health(db_path, tenant_id="team-a")
